...
def submit(request):
    ...
    players = Player.objects.all().order_by('-last_update')
    if players :
        last_update = players[0].last_update
    else :
        last_update = None
...
def nt_search(request):
    ...
    tag = u"DÉTAILS JOUEURS TROPHÉES CALENDRIER"
    parse_player = False
    player_set = []
    if request.method == "GET":
        return render(request,"players/submit.html",{"form":form,
        ...
        for line in text :
            ...
            if line.startswith(u"Toolbox") :
                parse_player = False
            if parse_player is True and not line.startswith("Age") and not line.startswith("Taille") and not line.startswith(u"Équipe") and not re.match(regexp,line.strip()) :
                player = {}
                player["full_name"] = line.strip()
